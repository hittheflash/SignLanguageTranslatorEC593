...
from frame import dir2tensor, images_normalize, frames_show
...
class FramesGenerator(keras.utils.Sequence):
    ...
    def __init__(self, sPath:str,
        ...
        print('self.dfVideos:',self.dfVideos)
    ...
    def __data_generation(self, seVideo:pd.Series) -> (np.array(float), int):
        ...
        ar_nFrames = dir2tensor(seVideo.sFrameDir)

        # only use the first nChannels (typically 3, but maybe 2 for optical flow)
        ar_nFrames = ar_nFrames[..., 0:self.nChannels]
        """
        ar_fFrames = images_normalize(ar_nFrames, 
                                      self.nFrames, 
                                      self.nHeight, 
                                      self.nWidth, 
                                      bRescale = True)
        """
        ar_fFrames = images_normalize(arFrames=ar_nFrames, 
                                      nFrames=self.nFrames, 
                                      h_w_target=(self.nHeight, self.nWidth), 
                                      bRescale = True)
